...
This article was originally posted on [www.danielnorris.co.uk](http://www.danielnorris.co.uk). You can connect with me on Twitter @[danielpnorris](https://twitter.com/danielpnorris).
...
There were three main reasons for me why I ended up choosing Gatsby compared to a many of the other Static Site Generators out there like Jekyll, Next.js, Hugo or even a SSG at all.
...
There are a lot of starter templates that are accessible from the Gatsby website which enable you to get off the ground running with a ready-made blog or portfolio in a couple clicks. What that doesn't do is break down how it works and how you could make one yourself. If you're more interested in getting sh\*t done than how it works, then I recommend taking a look at the starters [here](https://www.gatsbyjs.com/starters/?v=2).

We're going to build a basic portfolio site that looks like the one below. **INSERT IMAGE** We'll go through how to setup and configure your project to use Tailwind CSS, query and present MDX data sources using GraphQL, add transitions and animations using Framer and later deploy to Netlify.
...
Lets go and create your first page. Create a `src/pages/index.js` file in your root directory. It should look like this:

![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/91d017eb-73c4-42d9-a0ee-aa8967be2a78/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/91d017eb-73c4-42d9-a0ee-aa8967be2a78/Untitled.png)
...
Then import the directives and include PurgeCSS's whitelist selectors for best practice.
...
We're used the Gatsby `Link` component to route internally and then iterated over our destructured config file to create our nav links and paths.
...
We're using svg icons from [https://simpleicons.org/](https://simpleicons.org/). Copy the svg tag for a Github icon and include it in your Github icon sub component. Then do the same the remaining social media accounts you set up in your site config file.
...
We're going to need to load an image for our about page, so we'll use Gatsby's built-in functionality. It provides lazy-loading, image optimisation and additional processing features like blur-up and svg outlining with minimal effort.
...
Now, let's extend our GraphQL query on our `index.js` page to include data for our about page. You'll also to import and use the new About component. Make these changes to your `index.js` file.
...
Let's fix that now and import a component called `MDXProvider` which will allow us to define markdown elements. You could choose to link this up to already defined React components but we're just going to do it inline. Your `Layout.js` file should now look like this.
...
If we take a look at the `Header.js` component we can see below that the nav bar is looking a bit cluttered. Ideally what we would do here is add a hamburger menu button but we're going to keep this simple and add some breakpoints.
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/bd46abe7-91aa-4aca-85ab-6b5e32f1a9da/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/bd46abe7-91aa-4aca-85ab-6b5e32f1a9da/Untitled.png)
...
Finally, contact component will look like this.
...
Let's do the same with the `Hero.js` component. Except this time, we'll add an additional `transition` prop to each element with an incremental delay to make the animate stagger. Your final `Hero.js` component should look like this.
...
If you head over to [https://app.netlify.com/](https://app.netlify.com/) and choose "New site from git" and you'll be faced with the options below. Choose your git provider.

![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/73fa1ec2-17e0-4836-bb6b-a75323d1032b/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/73fa1ec2-17e0-4836-bb6b-a75323d1032b/Untitled.png)
...
You have a basic scaffold from which you can go ahead and extend as you see fit. If you've found this tutorial helpful, then please let me know. You can connect with me on Twitter at @danielpnorris.
